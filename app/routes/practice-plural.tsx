...
export default function PracticePlural() {
  ...
  const handleCheckAnswer = () => {
    ...
    const correct =
      normalized === correctNormalized || normalized === correctWithDie;
  ...
  const getHint = () =>
    !currentWord?.plural ? "" : `变化规则：${currentWord.plural}`;
  const progress =
    pluralWords.length > 0
      ? ((currentIndex + 1) / pluralWords.length) * 100
      : 0;
        ...
        <main className="flex-1 flex flex-col items-center justify-center px-4 py-8">
          ...
          <div className="flex flex-col gap-3 w-full max-w-xs">
            <button
              ...
              className="flex items-center justify-center gap-2 py-4 bg-teal-600 text-white rounded-2xl font-semibold cursor-pointer"
            >
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 mb-6 text-center">
          ...
          <div className="bg-teal-50 dark:bg-teal-900/20 rounded-xl p-6 mb-4">
            ...
            <p className="text-3xl font-bold text-gray-900 dark:text-gray-100 mb-2">
              {currentWord.article && (
                <span className="text-gray-500">{currentWord.article} </span>
              )}
              {currentWord.word}
            </p>
            ...
            <>
              ...
              <div className="flex gap-4 justify-center text-sm mt-2">
                <button
                  ...
                  className="flex items-center gap-1 text-gray-500 hover:text-teal-600 cursor-pointer"
                >
                  <Lightbulb className="w-4 h-4" />
                  {showHint ? "隐藏提示" : "显示提示"}
                </button>
      ...
      {isCorrect === null ? (
        ...
        <footer className="sticky bottom-0 bg-white/95 dark:bg-gray-900/95 backdrop-blur-xl border-t border-gray-100 dark:border-gray-800">
          <div className="px-4 py-3">
            <button
              onClick={handleNext}
              className="w-full flex items-center justify-center gap-2 py-4 bg-teal-600 text-white rounded-2xl font-semibold cursor-pointer"
            >
        ...
        <div className="bg-blue-50 dark:bg-blue-900/20 rounded-xl p-4">
          <div className="flex items-start gap-2">
            ...
            <div className="text-xs text-gray-600 dark:text-gray-400 space-y-1">
              <p>
                <span className="font-bold">-e</span>: 加 -e（Tag → Tage）
              </p>
              <p>
                <span className="font-bold">-er</span>: 加 -er（Kind → Kinder）
              </p>
              <p>
                <span className="font-bold">¨-e</span>: 元音变音 + 加后缀（Buch
                → Bücher）
              </p>
            </div>
          </div>
