...
export default function PracticeArticles() {
  ...
  const progress =
    nounWords.length > 0 ? ((currentIndex + 1) / nounWords.length) * 100 : 0;
        ...
        <main className="flex-1 flex flex-col items-center justify-center px-4 py-8">
              ...
              {(["der", "die", "das"] as const).map((article) => {
                ...
                return (
                  <div key={article} className="flex items-center gap-3">
                    <span
                      className={`w-10 font-bold text-sm ${
                        article === "der"
                          ? "text-blue-600"
                          : article === "die"
                          ? "text-pink-600"
          ...
          <div className="flex flex-col gap-3 w-full max-w-xs">
            <button
              ...
              className="flex items-center justify-center gap-2 py-4 bg-indigo-600 text-white rounded-2xl font-semibold cursor-pointer"
            >
            ...
            >
              ...
              {!isCorrect && (
                <p className="text-sm text-gray-500 mt-2">
                  你选择的是：
                  <span className="font-bold">{selectedArticle}</span>
                </p>
              )}
            </div>
          )}
      ...
      {selectedArticle !== null && (
        <footer className="sticky bottom-0 bg-white/95 dark:bg-gray-900/95 backdrop-blur-xl border-t border-gray-100 dark:border-gray-800">
          <div className="px-4 py-3">
            <button
              onClick={handleNext}
              className="w-full flex items-center justify-center gap-2 py-4 bg-indigo-600 text-white rounded-2xl font-semibold cursor-pointer"
            >
        ...
        <div className="bg-amber-50 dark:bg-amber-900/20 rounded-xl p-4">
          <div className="flex items-start gap-2">
            ...
            <div className="text-xs text-gray-600 dark:text-gray-400 space-y-1">
              ...
              <p>
                <span className="font-bold text-purple-600">das</span>:
                中性，-chen/-lein 结尾
              </p>
            </div>
          </div>
